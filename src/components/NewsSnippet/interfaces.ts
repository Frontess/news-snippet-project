...
export interface IData_TrafficItem {
  ...
  count: number; // объём траффика для указанной страны доля/процент
}

export interface IData_SnippetNews {
  ID: number; // идентификатор новости
  TI: string; // заголовок новости
  AB: string; // содержимое новости (полное)
  ...
  SENT: string; // сантимент новости (например, 'positive', 'negative', 'neutral')
  TRAFFIC: IData_TrafficItem[]; // траффик из стран
  FAV: string; // ссылка на иконку (может быть относительной)
  HIGHLIGHTS: string[]; // блоки содержимого новости с ключевыми словами <kw>...</kw>
}
