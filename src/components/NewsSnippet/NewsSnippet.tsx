...
import {
  ...
  IData_TagItem,
  IData_TrafficItem,
} from "./interfaces"; // Импорт интерфейсов
...
const NewsSnippet: React.FC<NewsSnippetProps> = ({ data }) => {
  ...
  const faviconUrl = FAV?.startsWith("/")
    ? `${window.location.origin}${FAV}`
    : FAV;

  const visibleTags = showAllTags ? KW : KW?.slice(0, MAX_VISIBLE_TAGS);
  const hiddenTagsCount =
    KW?.length > MAX_VISIBLE_TAGS ? KW.length - MAX_VISIBLE_TAGS : 0;
  ...
  return (
    <Card className="news-snippet-card" bordered={false}>
        ...
        <Space className="snippet-actions" size="small">
          ...
          <Tooltip title="Copy link">
            <Button
              type="text"
              icon={<CopyOutlined />}
              shape="circle" /* onClick={() => onCopy?.(data)} */
            />
          </Tooltip>
          ...
          <Tooltip title="Save">
            <Button
              type="text"
              icon={<StarOutlined />}
              shape="circle" /* onClick={() => onSave?.(data)} */
            />
          </Tooltip>
        </Space>
      </div>
      ...
      <Button
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="original-source-button"
      >
        Original Source
      </Button>
    </Card>
